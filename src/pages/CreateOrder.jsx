            ...
            <div className="main__Cart">
                ...
                <div className="box__Cart">
                    ...
                    <div className="boxAddress__detail">
                        <span className="namePhone">{user ? user.name : ''} - {user ? user.phone : ''}</span>
                    ...
                    {cart.products.map((item, index) => (
                        <div className="cart_detail" key={index}>
                            <span className="flex-4">
                                <img src={item.product.image} alt="" className="thumnail-img" />

                                <span>{item.product.name}</span>
                            </span>
                            <span className="flex-2">{item.product.name}</span>
                            <span className="flex-1">
                                <NumberFormat value={item.product.price} displayType={'text'} thousandSeparator={true} prefix={''} />
                            </span>
                            <span className="flex-1">{item.quantity}</span>
                            <span className="flex-2">
                                <NumberFormat value={item.product.price * item.quantity} displayType={'text'} thousandSeparator={true} prefix={''} />
                            </span>
                        </div>
                    ))}
                </div>
                ...
                <div className="cart_pay">
                    <div className="cart_pay_lbl">
                        ...
                        <span><NumberFormat value={isSum} displayType={'text'} thousandSeparator={true} prefix={'vnđ '} /></span>
                    </div>

                    <div className="cart_group_btn">
                        <span>Nhấn "Đặt hàng" đồng nghĩa với việc bạn đồng ý tuân theo Điều khoản Shopee</span>

                        <button className="btnCreateOrder" onClick={() => { createOrder(cart.products, user, isSum) }}>
                            Đặt hàng
                        </button>
                    </div>
                </div>
